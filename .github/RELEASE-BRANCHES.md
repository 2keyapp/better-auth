...
**Never delete** `release`, `release-native`, or `release-scim`. Deleting them breaks every consumer that pins those refs (including `2key-billing`). Pinning `#main` also breaks installs because the monorepo root has no published `dist/` layout.
