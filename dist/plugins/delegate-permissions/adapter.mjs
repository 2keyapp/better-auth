...
function getDelegatePermissionsAdapter(adapter, serviceId) {
	return {
		...
		async getEnrollInviteByToken(inviteToken) {
			return adapter.findOne({
				model: "dpEnrollInvite",
				where: [{
					field: "inviteToken",
		...
		async redeemEnrollInvite(id) {
			...
			const maxUses = typeof row.maxUses === "number" && row.maxUses >= 0 ? row.maxUses : 1;
			const usedCount = typeof row.usedCount === "number" && row.usedCount >= 0 ? row.usedCount : 0;
			...
			return adapter.update({
				...
				update: {
					usedCount: next,
					...exhausted ? { consumedAt: /* @__PURE__ */ new Date() } : {}
				}
			});
		}
	};
