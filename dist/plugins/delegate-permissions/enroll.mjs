...
function inviteTtlSeconds(expiresIn, defaultTtl, maxTtl) {
	const sec = expiresIn ?? defaultTtl;
	if (!Number.isFinite(sec) || sec <= 0 || sec > maxTtl) throw APIError.from("BAD_REQUEST", {
...
function inviteIsExhausted(maxUses, usedCount) {
	return maxUses > 0 && usedCount >= maxUses;
}
async function requireOpenInvite(dp, inviteToken) {
	const row = await dp.getEnrollInviteByToken(inviteToken);
	if (!row) throw APIError.from("NOT_FOUND", DELEGATE_PERMISSIONS_ERROR_CODES.INVITE_NOT_FOUND);
	const expiresAt = asDate(row.expiresAt);
	if (Number.isNaN(expiresAt.getTime()) || expiresAt.getTime() <= Date.now()) throw APIError.from("BAD_REQUEST", DELEGATE_PERMISSIONS_ERROR_CODES.INVITE_EXPIRED);
	const maxUses = asCount(row.maxUses, 1);
	const usedCount = asCount(row.usedCount, 0);
	if (row.consumedAt || inviteIsExhausted(maxUses, usedCount)) throw APIError.from("BAD_REQUEST", DELEGATE_PERMISSIONS_ERROR_CODES.INVITE_USED);
	return {
		id: row.id,
...
function createEnrollEndpoints(opts) {
	const inviteMaxExpiresIn = opts.inviteMaxExpiresIn ?? 2592e3;
	const inviteExpiresIn = Math.min(opts.inviteExpiresIn ?? 604800, inviteMaxExpiresIn);
	const inviteMaxUses = opts.inviteMaxUses ?? 1;
	...
	return {
		dpEnrollCreate: createAuthEndpoint("/delegate-permissions/enroll-create", {
			...
			metadata: { openapi: { description: "Create a pending enrollment CSR (machine, zone authority, or interim admin). Optional inviteToken binds the CSR to an invited entity." } }
		}, async (ctx) => {
			const dp = dpOf(ctx.context.adapter);
			await ensureCatalog(dp, opts.configuredSeed);
			const invite = ctx.body.inviteToken ? await requireOpenInvite(dp, ctx.body.inviteToken) : null;
			let entityId = (ctx.body.entityId ?? invite?.entityId ?? "").toLowerCase();
			if (invite) {
				if (entityId && entityId !== invite.entityId) throw APIError.from("BAD_REQUEST", DELEGATE_PERMISSIONS_ERROR_CODES.INVITE_MISMATCH);
				entityId = invite.entityId;
			}
			if (!entityId) throw APIError.from("NOT_FOUND", DELEGATE_PERMISSIONS_ERROR_CODES.ENTITY_NOT_FOUND);
			...
			if (invite) {
				if (!await dp.redeemEnrollInvite(invite.id)) throw APIError.from("BAD_REQUEST", DELEGATE_PERMISSIONS_ERROR_CODES.INVITE_USED);
			}
		...
		dpEnrollInviteCreate: createAuthEndpoint("/delegate-permissions/enroll-invite", {
			method: "POST",
			...
			metadata: { openapi: { description: "Create a push-invite token that authorizes a device to submit a CSR for an entity" } }
		}, async (ctx) => {
			const dp = dpOf(ctx.context.adapter);
			await ensureCatalog(dp, opts.configuredSeed);
			...
			const entity = await dp.getEntity(entityId);
			if (!entity) throw APIError.from("NOT_FOUND", DELEGATE_PERMISSIONS_ERROR_CODES.ENTITY_NOT_FOUND);
			const kind = normalizeEnrollKind(ctx.body.kind ?? ctx.body.role ?? "machine_target");
			if (!isMachineKind(kind)) throw APIError.from("BAD_REQUEST", {
				message: "enroll invites are for machine_target or machine_source only",
				code: "INVITE_KIND_UNSUPPORTED"
			});
			assertKindAllowedForPackage(entity.package, kind);
			const maxUses = ctx.body.maxUses ?? inviteMaxUses;
			const expiresAt = new Date(Date.now() + inviteTtlSeconds(ctx.body.expiresIn, inviteExpiresIn, inviteMaxExpiresIn) * 1e3);
		...
		dpEnrollInviteGet: createAuthEndpoint("/delegate-permissions/enroll-invite", {
			method: "GET",
			query: z.object({ inviteToken: z.string().min(1) }),
			metadata: { openapi: { description: "Look up an enrollment invite without consuming it" } }
		}, async (ctx) => {
			const invite = await requireOpenInvite(dpOf(ctx.context.adapter), ctx.query.inviteToken);
			return {
				inviteId: invite.id,
				entityId: invite.entityId,
				kind: invite.role,
