...
async function createSelfSignedCaPem(privateJwk, commonName, notAfterDays) {
	setX509Crypto();
	...
	const notAfter = new Date(notBefore.getTime() + (notAfterDays ?? 3650) * DAY_MS);
	return {
...
async function createPlatformRootPem(privateJwk, commonName = DEFAULT_PLATFORM_CN, notAfterDays) {
...
async function loadPlatformCaMaterial(input) {
	setX509Crypto();
	const built = await createPlatformRootPem(input.privateJwk, input.commonName ?? DEFAULT_PLATFORM_CN, input.notAfterDays);
...
async function generateEphemeralPlatformCa(commonName, notAfterDays) {
	setX509Crypto();
