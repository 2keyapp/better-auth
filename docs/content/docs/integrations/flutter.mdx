...
<Steps>
  ...
  <Step>
    ...
    Install both the Better Auth package and the Flutter plugin into your server application.

    ```package-install
    better-auth @better-auth/flutter
    ```
  </Step>
  ...
  <Step>
    ...
    Add the Dart client to your Flutter / Dart project via Git (until published on [pub.dev](https://pub.dev)):
...
Provide a `sessionLauncher` that opens an auth session in the system / in-app browser and returns the redirect URL (for example via [`flutter_web_auth_2`](https://pub.dev/packages/flutter_web_auth_2)). Relative `callbackURL` values are converted to deep links using your scheme.
