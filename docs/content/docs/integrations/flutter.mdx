...
<Steps>
  ...
  <Step>
    ...
    Install from GitHub release branches — core on **`release`**, server plugin on **`release-flutter`**:
    ...
    <Callout>
      Release branch: **`release-flutter`** · See [Install from GitHub](/docs/guides/github-installation) for the full package index.
    </Callout>
  </Step>
  ...
  <Step>
    ...
    The Dart client is **not** on a release branch. Pub installs it from the monorepo on **`main`** (or a pinned commit) with `path`:
    ...
    For production, pin `ref` to a commit SHA from `main` instead of tracking the branch head.

    <Callout type="warn">
      Do not use `#release-flutter` for the Dart client — that branch only contains the npm server plugin artifact.
    </Callout>
