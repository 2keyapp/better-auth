...
<Steps>
  <Step>
    ...
    Install from GitHub using the **`release-scim`** branch (and core Better Auth from **`release`**):
    ...
    Then run `pnpm install`, `npm install`, or `yarn`.

    <Callout>
      Release branch: **`release-scim`** · See [Install from GitHub](/docs/guides/github-installation) for all package refs.
    </Callout>
  </Step>
...
The SCIM plugin provides hooks for token generation and user provisioning lifecycle events.
...
<Callout>
  **Note**: All hooks support error handling. Throwing an error in `beforeSCIMTokenGenerated` prevents the token from being persisted.
</Callout>

#### User provisioning hooks

Use these hooks to react when an identity provider provisions or deprovisions users through SCIM.

* **`afterSCIMUserProvisioned`**: Runs after `POST /scim/v2/Users` successfully creates a user. The payload includes the provisioned `user`, the `scimProvider` associated with the bearer token, and the `externalId` when the IdP supplied one (otherwise derived from `userName`).
* **`afterSCIMUserDeprovisioned`**: Runs after:
  * An organization-scoped `DELETE /scim/v2/Users/:userId` removes the user's membership and provider account for that organization
  * A `PUT` or `PATCH` sets `active: false`, which deactivates the user and revokes their sessions (requires the [admin](/docs/plugins/admin) plugin)

Non-organization `DELETE` requests do not invoke this hook. Those requests unlink or delete the global user instead of org-scoped deprovisioning.

```ts
scim({
    afterSCIMUserProvisioned: async ({ user, scimProvider, externalId }) => {
        await notifyTeamOfNewMember({
            userId: user.id,
            organizationId: scimProvider.organizationId,
            externalId,
        });
    },
    afterSCIMUserDeprovisioned: async ({ user, scimProvider }) => {
        await revokeThirdPartyAccess({
            userId: user.id,
            organizationId: scimProvider.organizationId,
